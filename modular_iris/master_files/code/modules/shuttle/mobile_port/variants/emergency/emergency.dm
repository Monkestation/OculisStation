...
/obj/docking_port/mobile/emergency/proc/prevent_guns(mob/living/user)
	SIGNAL_HANDLER
	to_chat(user, span_warning("You wouldn't violate your oath of non-violence when the shuttle docks at SectCom, would you?")) // OCULIS EDIT, SectCommening 2, ORIGINAL: to_chat(user, span_warning("You wouldn't violate your oath of non-violence when the shuttle docks at centcom, would you?"))
...
/obj/docking_port/mobile/emergency/proc/prevent_grabs(mob/living/grabber, atom/grabbed_thingy)
	...
	if(!grabber.density && grabbed_thingy.density)
		to_chat(grabber, span_warning("You wouldn't violate your oath of non-violence when the shuttle docks at SectCom, would you?")) // OCULIS EDIT, SectCommening 2, ORIGINAL: to_chat(grabber, span_warning("You wouldn't violate your oath of non-violence when the shuttle docks at centcom, would you?"))
