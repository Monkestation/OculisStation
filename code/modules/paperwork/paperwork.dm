...
/obj/item/paperwork/examine_more(mob/user)
	...
	if(ishuman(user))
		...
		if(istype(viewer.mind?.assigned_role, stamp_job)) //Examining the paperwork as the proper job gets you some bonus details
			...
			if(stamped)
				. += span_info("It looks like these documents have already been stamped. Now they can be returned to Sectorial Command.") // OCULIS EDIT, SectCommening 2, ORIGINAL: .*Central Command.")
			else
...
/obj/item/paperwork/service/Initialize(mapload)
	...
	detailed_desc += span_info(" You begin scanning over the document. This is a standard Nanotrasen NT-435Z3 form used for requests to Sectorial Command.") // OCULIS EDIT, SectCommening 2, ORIGINAL: .*Central Command.")
...
/obj/item/paperwork/photocopy/examine_more(mob/user)
	...
	if(stamped)
		if(voided)
			...
			. += span_notice("The stamp on the front appears to be smudged and faded. Sectorial Command will probably still accept these, right?") // OCULIS EDIT, SectCommening 2, ORIGINAL: .*Central Command will probably still accept these, right?")
	else
...
/obj/item/paperwork/ancient/Initialize(mapload)
	...
	detailed_desc = span_notice("It's impossible to really tell how old these are or what they're for, but Sectorial Command might appreciate them anyway.") // OCULIS EDIT, SectCommening 2, ORIGINAL: .*Central Command might appreciate them anyway.")
