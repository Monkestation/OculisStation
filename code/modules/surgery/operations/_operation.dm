...
/datum/surgery_operation/proc/get_mob_surgery_speed_mod(mob/living/patient, mob/living/surgeon, tool)
	...
	if(HAS_TRAIT(patient, TRAIT_ANALGESIA))
		basemod *= 0.8
		to_chat(surgeon, span_smallnotice("You are able to work faster due to the patient's calm attitude!")) // NOVA EDIT ADDITION - Better feedback for the use of analgesia // OCULIS EDIT, NOVA ORIGINAL: to_chat(surgeon, span_notice("You are able to work faster due to the patient's calm attitude!"))
	return basemod
...
/datum/surgery_operation/proc/get_surgeon_surgery_speed_mod(mob/living/patient, mob/living/surgeon, tool)
	...
	if(quiet_environment)
		basemod *= 0.8
		to_chat(surgeon, span_smallnotice("You are able to work faster due to the quiet environment!")) // OCULIS EDIT, NOVA(?) ORIGINAL: to_chat(surgeon, span_notice("You are able to work faster due to the quiet environment!"))
