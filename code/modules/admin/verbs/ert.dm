...
/datum/admins/proc/make_emergency_response_team(datum/ert/ertemplate = null)
	...
	if(ertemplate.notify_players)
		priority_announce("Sectorial Command has responded to your request for a CODE [uppertext(ertemplate.code)] Emergency Response Team and have confirmed one to be enroute.", "ERT Request", ANNOUNCER_ERTYES) // OCULIS EDIT, SectCommening 2, ORIGINAL: .*Central Command has responded to your request for a CODE [uppertext(ertemplate.code)] Emergency Response Team and have confirmed one to be enroute.", "ERT Request", ANNOUNCER_ERTYES)
	// NOVA EDIT END
	return TRUE
