...
/obj/item/book/bible
	name = "bible"
	desc = "Mend the flesh and soul." /// OCULIS EDIT (prev. "Apply to head repeatedly.")
...
/obj/item/book/bible/proc/bless(mob/living/blessed, mob/living/user)
	...
	built_in_his_image.visible_message(span_notice("[user] mends [built_in_his_image] with the power of [deity_name]!")) /// OCULIS EDIT (prev. "[user] heals [built_in_his_image] with the power of [deity_name]!")
	to_chat(built_in_his_image, span_boldnotice("The power of [deity_name] mends you!")) /// OCULIS EDIT (prev. "May the power of [deity_name] compel you to be healed!")
	playsound(built_in_his_image, 'sound/effects/magic/staff_healing.ogg', 25, TRUE, -1) /// OCULIS EDIT (prev. SFX_PUNCH)
...
/obj/item/book/bible/attack(mob/living/target_mob, mob/living/carbon/human/user, list/modifiers, list/attack_modifiers, heal_mode = TRUE)
	...
	if(target_mob.stat == DEAD)
		if(GLOB.religious_sect?.sect_dead_bless(target_mob, user) == BLESSING_FAILED)
			target_mob.visible_message(span_danger("[user] fails to mend [target_mob]'s lifeless corpse with [src].")) /// OCULIS EDIT (prev. ("[user] smacks [target_mob]'s lifeless corpse with [src].")
			playsound(target_mob, 'sound/effects/magic/staff_healing.ogg', 25, TRUE, -1) /// OCULIS EDIT (prev. SFX_PUNCH)
		return
	...
	target_mob.visible_message(span_danger("[user] attempts to mend [target_mob] with [src]!"), \
			span_userdanger("[user] attempts to mend [target_mob] with [src]!")) /// OCULIS EDIT (prev. "[user] beats [target_mob] over the head with [src]!")
	playsound(target_mob, 'sound/effects/magic/staff_healing.ogg', 25, TRUE, -1) /// OCULIS EDIT (prev. SFX_PUNCH)
