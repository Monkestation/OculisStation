	...
	if(ishuman(user))
		...
		if(istype(human_user.ears, /obj/item/radio/headset))
			to_chat(user, span_notice("You hear something crackle in your ears for a moment before a voice speaks. \
				\"Please stand by for a message from Sectorial Command.  Message as follows: \ // OCULIS EDIT, SectCommening 2, ORIGINAL: \"Please stand by for a message from Central Command.  Message as follows: \
