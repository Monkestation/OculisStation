...
/datum/controller/subsystem/events/checkEvent()
	if(scheduled <= world.time)
		if(SSautotransfer.can_fire == TRUE && ((SSautotransfer.targettime - REALTIMEOFDAY) <= 90 SECONDS + CONFIG_GET(number/vote_period))) //IRIS EDIT: changed 15 minutes to 1.5
			log_game("ICES: Event cancelled due to precondition check. Reason: Pending autotransfer vote.")
			log_game("ICES: Info: AT [(SSautotransfer.targettime - REALTIMEOFDAY)] | VP [(90 SECONDS + CONFIG_GET(number/vote_period))]") //IRIS EDIT: changed 15 minutes to 1.5
